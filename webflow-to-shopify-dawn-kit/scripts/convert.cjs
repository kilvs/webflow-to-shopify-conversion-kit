#!/usr/bin/env node
/**
 * Webflow → Shopify bulk page-content extractor — DAWN VARIANT.
 *
 * Identical to the base kit's convert.cjs, with one safeguard:
 *   pages handled natively by Dawn (product, collection, cart, search,
 *   customer account) are SKIPPED even if added to PAGES below. Dawn's
 *   sections + templates for those are merged in by
 *   webflow-to-shopify-dawn-kit/scripts/merge-dawn-commerce.{sh,ps1}.
 *
 * For each non-skipped source HTML file in PAGES, slices content between
 * </header> and <footer> (the "main"), rewrites asset paths and internal
 * links to Shopify equivalents, and writes:
 ...
 *
 * Page-specific dynamic data wiring (blog/article — but NOT product/collection,
 * which are Dawn's job) is applied AFTER this script runs — see
 * CONVERSION_GUIDE.md §E.
 *
 * Edit PAGES + HREF_MAP below for your project, then:
 *   node webflow-to-shopify-dawn-kit/scripts/convert.cjs
 *
...
// ────────────────────────────────────────────────────────────────────────────
// DAWN-OWNED PAGES — skipped to avoid colliding with Dawn's commerce templates
// ────────────────────────────────────────────────────────────────────────────
// If your Webflow export uses different filenames for these page types, add
// them here so the safeguard catches them. Brand pages (homepage, about,
// contact, /pages/*, blog/news, articles, 404, password) are NOT in this list
// — they get converted normally from the Webflow source.
const DAWN_OWNED = new Set([
  // Product pages
  'product-page.html',
...
// Map every internal *.html link to its Shopify route. Dawn-owned routes are
// kept here so links FROM brand pages TO commerce pages still resolve.
const HREF_MAP = {
  ...
  'cart.html':               "{{ routes.cart_url }}",
  'search.html':             "{{ routes.search_url }}",
  'account.html':            "{{ routes.account_url }}",
  'login.html':              "{{ routes.account_login_url }}",
  'login-signup.html':       "{{ routes.account_login_url }}",
  'register.html':           "{{ routes.account_register_url }}",
...
for (const p of PAGES) {
  if (DAWN_OWNED.has(p.html)) {
    console.warn(`SKIP (Dawn-owned): ${p.html} — Dawn supplies this template via merge-dawn-commerce`);
    skipped++;
    continue;
  }
...
console.log(`\nDone. ${ok} converted, ${skipped} skipped (Dawn-owned), ${missing} missing.`);
